...
public class ItemSupremiumArmor extends ItemArmor implements IRepairMaterial {
    ...
    public void addInformation(ItemStack stack, @Nullable World world, List<String> tooltip, ITooltipFlag advanced) {
        ...
        if (ModConfig.confSupremiumFlight) {
            tooltip.add(Tooltips.SET_BONUS + Colors.RED + Tooltips.FLIGHT);
        }
        ...
        if (tag.hasKey(ArmorType.ARMOR_TYPE)) {
            tooltip.add(Tooltips.CHARM_SLOT + Colors.RED +
                    ArmorType.byIndex(tag.getInteger(ArmorType.ARMOR_TYPE)).getLocalizedName());
        } else {
    ...
    public static boolean isFullSet(EntityPlayer player) {
        ...
        return !head.isEmpty() && head.getItem() instanceof ItemSupremiumArmor && !chest.isEmpty() &&
                chest.getItem() instanceof ItemSupremiumArmor && !legs.isEmpty() &&
                legs.getItem() instanceof ItemSupremiumArmor && !feet.isEmpty() &&
                feet.getItem() instanceof ItemSupremiumArmor;
    }
                ...
                if (playersWithSpeed.contains(key)) {
                    if (hasSpeed) {
                        ...
                        if (player.onGround || flying || swimming) {
                            ...
                            float speed = 0.1f * (flying ? 0.6f : 1.0f) * (sneaking ? 0.1f : 1.0f) *
                                    (!sprinting ? 0.6F : 1.2F);

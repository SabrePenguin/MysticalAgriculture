    ...
    public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, ITooltipFlag advanced) {
        ...
        if (OreDictionary.itemMatches(getRepairMaterial(), ModItems.itemCrafting.itemSupremiumIngot, false)) {
            ...
            if (tag.hasKey(ToolType.TOOL_TYPE)) {
                tooltip.add(Tooltips.CHARM_SLOT + Colors.RED +
                        ToolType.byIndex(tag.getInteger(ToolType.TOOL_TYPE)).getLocalizedName());
            } else {
                ...
                if (tag.getInteger(ToolType.TOOL_TYPE) == ToolType.RAINBOW.getIndex()) {
                    ...
                    if (entity instanceof IShearable) {
                        ...
                        if (target.isShearable(itemstack, entity.getEntityWorld(), pos)) {
                            List<ItemStack> drops = target.onSheared(itemstack, entity.getEntityWorld(), pos,
                    ...
                    List<EntityLivingBase> entities = player.getEntityWorld().getEntitiesWithinAABB(
                            EntityLivingBase.class, entity.getEntityBoundingBox().grow(1.5D, 0.25D, 1.5D));
        ...
        for (BlockPos aoePos : blocks) {
            ...
            if (block instanceof IShearable) {
                ...
                if (target.isShearable(itemstack, player.getEntityWorld(), aoePos)) {
                    List<ItemStack> drops = target.onSheared(itemstack, player.getEntityWorld(), aoePos,
                            EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack));
                    Random rand = new Random();
                    ...
                    for (ItemStack stack : drops) {
                        ...
                        EntityItem entityitem = new EntityItem(player.getEntityWorld(), (double) aoePos.getX() + d,
                                (double) aoePos.getY() + d1, (double) aoePos.getZ() + d2, stack);

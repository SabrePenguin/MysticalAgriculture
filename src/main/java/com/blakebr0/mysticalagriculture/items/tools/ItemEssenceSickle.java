...
public class ItemEssenceSickle extends ItemBase implements IRepairMaterial {
    ...
    public boolean harvest(ItemStack stack, int radius, World world, BlockPos pos, EntityPlayer player) {
        ...
        if (!canHarvest(world, pos, false, stack, player) || !(state.getMaterial() == Material.LEAVES ||
        ...
        if (radius > 0) {
            int used = 0;
            Iterable<BlockPos> blocks = BlockPos.getAllInBox(pos.add(-radius, -radius, -radius),
                    pos.add(radius, radius, radius));
            for (BlockPos aoePos : blocks) {
                if (aoePos != pos) {
                    IBlockState aoeState = world.getBlockState(aoePos);
                    if (aoeState.getBlockHardness(world, aoePos) <= hardness + 5.0F) {
                        if (aoeState.getMaterial() == Material.LEAVES || aoeState.getMaterial() == Material.PLANTS ||
                                aoeState.getMaterial() == Material.VINE) {
    ...
    private boolean canHarvest(World world, BlockPos pos, boolean extra, ItemStack stack, EntityPlayer player) {
        ...
        boolean harvest = (ForgeHooks.canHarvestBlock(block, player, world, pos) ||
                this.canHarvestBlock(state, stack)) &&
                (!extra || this.getDestroySpeed(stack, world.getBlockState(pos)) > 1.0F);

...
public class TileEntitySeedReprocessor extends TileEntityUtil
    ...
    public void update() {
        ...
        if (time_left <= 0) {
            if (!input.isEmpty() && (processing.isEmpty())) {
                if (!ReprocessorManager.getOutput(input).isEmpty() &&
                        (output.isEmpty() || ReprocessorManager.getOutput(input).isItemEqual(output))) {
                    if (processing.isEmpty()) {
        ...
        if (time_left <= 0) {
            if (progress >= target && !processing.isEmpty() && (output.isEmpty() || (output.isItemEqual(processing) &&
                    output.getCount() + processing.getCount() <= output.getMaxStackSize()))) {
                if (output.isEmpty()) {

...
public abstract class TileEssenceReprocessor extends TileEntityUtil
                                             implements ISidedInventory, ITickable, ICapabilityProvider {
    ...
    public void update() {
        ...
        int fuelPerTick = Math.min(Math.min(this.fuelLeft, this.getFuelUsage() * 2),
                this.getFuelCapacity() - this.fuel);
        ...
        if (this.fuel >= this.getFuelUsage()) {
            ...
            if (!input.isEmpty()) {
                ...
                if (!recipeOutput.isEmpty() &&
                        (output.isEmpty() || StackHelper.canCombineStacks(output, recipeOutput))) {
                    this.progress++;
    ...
    public boolean isUsableByPlayer(EntityPlayer player) {
        return this.world.getTileEntity(this.getPos()) == this &&
                player.getDistanceSq(this.getPos().add(0.5D, 0.5D, 0.5D)) <= 64.0D;
    }
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        if (index == 2) {
            ...
            return TileEntityFurnace.isItemFuel(stack) ||
                    SlotFurnaceFuel.isBucket(stack) && itemstack.getItem() != Items.BUCKET;
        }
    }

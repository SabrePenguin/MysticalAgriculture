...
public class BlockIntermediumFurnace extends BlockEssenceFurnace {
    ...
    public static void setState(boolean active, World world, BlockPos pos) {
        ...
        if (active) {
            world.setBlockState(pos, ModBlocks.blockIntermediumFurnaceActive.getDefaultState().withProperty(FACING,
                    state.getValue(FACING)), 3);
            world.setBlockState(pos, ModBlocks.blockIntermediumFurnaceActive.getDefaultState().withProperty(FACING,
                    state.getValue(FACING)), 3);
        } else {
            world.setBlockState(pos,
                    ModBlocks.blockIntermediumFurnace.getDefaultState().withProperty(FACING, state.getValue(FACING)),
                    3);
            world.setBlockState(pos,
                    ModBlocks.blockIntermediumFurnace.getDefaultState().withProperty(FACING, state.getValue(FACING)),
                    3);
